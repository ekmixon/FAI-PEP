...
class ADB(PlatformUtilBase):
    ...
    def reboot(self):
        try:
            ...
            getLogger().error(f"Unknown exception {e}")
            return False
    ...
    def shell(self, cmd, **kwargs):
        dft = kwargs.pop("default") if "default" in kwargs else None
    ...
    def setFrequency(self, target):
        ...
        if ret.find("not found") >= 0:
            getLogger().info(f"Device {self.device} is not rooted.")
            return
        ...
        for cpu in cpus:
            ...
            if isinstance(target, dict):
                freq_target = target[cpu] if cpu in target else "mid"
    ...
    def _setOneCPUFrequency(self, cpu, freq_target):
        ...
        scaling_governor = f"{directory}cpufreq/scaling_governor"
        ...
        avail_freq = f"{directory}cpufreq/scaling_available_frequencies"
        ...
        if freq_target == "max":
            ...
            freq = freq_target
        minfreq = f"{directory}cpufreq/scaling_min_freq"
        self.su_shell([f'"echo {freq} > {minfreq}"'])
        maxfreq = f"{directory}cpufreq/scaling_max_freq"
        self.su_shell([f'"echo {freq} > {maxfreq}"'])
        curr_speed = f"{directory}cpufreq/scaling_cur_freq"
        set_freq = self.su_shell(["cat", curr_speed]).strip()
        assert set_freq == freq, f"Unable to set frequency {freq_target} for {cpu}"
